...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mm(ggml_metal_library_t lib, const ggml_tensor * op) {
    ...
    // TQ4_1S uses float simdgroup ops: sa=8192 + sb=4096 = 12288
...
// TQ4_1S rotated variant: uses dequantize_*_rotated (no inverse RHT)
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mm_tq_rotated(ggml_metal_library_t lib, const ggml_tensor * op) {
    ...
    // TQ4_1S uses float simdgroup ops → needs 12288 shmem
    res.smem = 8192;

    return res;
}

// TQ4_1S rotated MUL_MAT_ID variant
...
// TQ activation pre-rotation pipeline (shared by TQ types)
